...
void main() {
  testWidgets('settings screen exposes editable model, library, and language controls', (
    WidgetTester tester,
  ) async {
    ...
    expect(find.text('Lokasi library'), findsOneWidget);
    expect(find.text('Bahasa'), findsOneWidget);

    await tester.enterText(find.byType(TextFormField), '/tmp/trascribe');
    await tester.testTextInput.receiveAction(TextInputAction.done);
    await tester.pumpAndSettle();

    expect(bridge.savedSettings.libraryPath, '/tmp/trascribe');
    ...
    expect(bridge.savedSettings.theme, AppThemeMode.dark);
  });
}
